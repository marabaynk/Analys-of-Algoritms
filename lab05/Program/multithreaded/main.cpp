...
#pragma optimize("", off)
void work_1_3(IntMatrix im1, mutex& mutex_im1,
              size_t im1_rows_begin,
              size_t im1_rows, mutex& mutex_im1_rows,
              size_t im2_rows_2, mutex& mutex_im2_rows_2,
              std::vector<int> &row, mutex& mutex_row,
              mutex& mutex_out){
    ...
    //mutex_im1_rows.unlock();

    //mutex_im2_rows_2.lock();
    size_t j_size = im2_rows_2;
    //mutex_im2_rows_2.unlock();
...
#pragma optimize("", off)
void work_2_3(IntMatrix im2, mutex& mutex_im2,
              size_t im2_columns_begin,
              size_t im2_columns, mutex& mutex_im2_columns,
              size_t im2_rows_2, mutex& mutex_im2_rows_2,
              std::vector<int> &column, mutex& mutex_column,
              mutex &mutex_out){

    //mutex_im2_columns.lock();
    size_t i_begin = im2_columns_begin;
    size_t i_size = im2_columns;
    //mutex_im2_columns.unlock();

    //mutex_out.lock();
    //std::cout << "\n" << "wait my work_2_3 thread!" << im2_rows_2;
    //mutex_out.unlock();
    ...
    for (size_t i = i_begin; i < i_size; i++) { // 2 + im2_columns * (
        for (size_t j = 0; j < j_size; j++) { // 2 + im2_rows_2 * (
            //std::lock(mutex_im2, mutex_column);
...
void work_3_3(IntMatrix im1, IntMatrix im2,
              mutex& mutex_im1, mutex& mutex_im2,
              std::vector<int> c, mutex& mutex_column,
              std::vector<int> r, mutex& mutex_row,
              size_t im1_rows_begin,
              size_t im1_rows,
              size_t im2_columns,
              size_t im2_rows_2,
              IntMatrix &return_me, mutex& mutex_return_me)
{
    ...
    for (size_t i = i_begin; i < i_size; i++) {
        //mutex_return_me.lock();
        return_me.m[i].clear();
        //mutex_return_me.unlock();

        for (size_t j = 0; j < j_size; j++) {
            //mutex_return_me.lock();
            return_me.m[i].push_back(0);

            //std::lock(mutex_column, mutex_row);
            ...
            for (size_t k = 0; k < k_size; k++) {
                //std::lock(mutex_return_me, mutex_im1, mutex_im2);
                return_me.m[i][j] += (
